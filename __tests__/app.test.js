...
describe("GET /api/sir-not-appearing/in-this-app", () => {
  it("should return a 404 error for a non-existent endpoint", () => request(app).get("/api/sir-not-appearing/in-this-app").expect(404));
});
...
describe("GET /api", () => {
  it("should return a JSON representation of all available API endpoints", () => {
    ...
    return fs.readFile(filePath).then((file) => request(app)
      .get("/api")
      .expect(200)
      .then(({ body }) => {
